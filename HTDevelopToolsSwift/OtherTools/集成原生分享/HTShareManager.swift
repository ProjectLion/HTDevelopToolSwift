...
class HTShareManager: NSObject {
    ...
    func shareTo(toType type: HTShareType, dataType: HTShareDataType, url: String, thumbImg: UIImage) {
        ...
        case .video:
            // 先上传到七牛
            uploadVideo(type: type, path: url, thumbImg: thumbImg)
        case .photo:
    ...
    ///   - type: 分享的类型
    ///   - path: 视频文件路径
    private func uploadVideo(type: HTShareType, path: String, thumbImg: UIImage) {
        
        /// 先查询数据库,看当前文件是否上传过
        let model = DataBaseTool.share.selectVideoData(videoID: path.components(separatedBy: "/").last!)
        if model.video_uploadStatus != "0" {    // 0为未上传
            // 已经上传过，直接分享
            self.shareVideoTo(toType: type, url: model.video_uploadStatus, thumbImg: thumbImg)
            return
        }
        
        // 先从自己的服务器获取相关的信息
        Alamofire.request(GetQNTokenApi, method: .get, parameters: nil, encoding: JSONEncoding.default, headers: nil).responseJSON { (response) in
            if response.result.isSuccess {
                let data = response.result.value as! Dictionary<String, Any>
//                print("HT------>DEBUG: \(data)")
                if data["status"] as! Int == 1 {
                    let dataDict = data["data"] as! Dictionary<String, String>
                    
                    let upToken = dataDict["upToken"]
//                    let upUrl = dataDict["upUrl"]
                    let shareURL = dataDict["shareUrl"]
                    
                    let md5Str = MGFileManager.share.md5File(url: URL(fileURLWithPath: path))
                    let progressView = MGLoadingView(progress: "0%", tip: "视频上传中...")
                    DispatchQueue.global().async {
                        ...
                        let uploadOption = QNUploadOption(mime: nil, progressHandler: { (key, progress) in
                            // 上传进度
//                            print("HT------>DEBUG: \(progress*100)")
                            DispatchQueue.main.async {
                                progressView.progress = String(format: "%.0f", progress*100)
                            }
                        }, params: ["x:md5": md5Str!], checkCrc: true, cancellationSignal: nil)
                        
                        qnManager?.putFile(path, key: path.components(separatedBy: "/").last!, token: upToken!, complete: { (info, key, resp) in
                            /// 上传完成
                            // 更新数据库 改变上传状态
//                            print("HT------>DEBUG: 上传完成")
                            
                            DataBaseTool.share.updataVideoUploadStatus(status: shareURL! + md5Str!, videoID: path.components(separatedBy: "/").last!)
                            DispatchQueue.main.async {
                                progressView.tip = "上传完成"
                                progressView.hide()
                                self.shareVideoTo(toType: type, url: shareURL! + md5Str!, thumbImg: thumbImg)
                            }
                        }, option: uploadOption!)
                    }
                }
                ...
                DispatchQueue.main.async {
                    MGMessageAlert.share.showAlert(text: "请检查网络连接")
                }
            }
        }
    }
